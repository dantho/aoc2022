...
pub fn gen1(input: &str) -> Terrain {
    let mut tmap:  Vec<Vec<char>>= input.lines()
        .map(|line|line.chars().filter(|c|c.is_alphabetic()).collect()).collect();
    ...
    let tmap = tmap.iter()
        .map(|row|row.iter()
            .map(|&c|c as u8 - 'a' as u8).collect::<Vec<_>>()
        ).collect::<Vec<_>>();
...
pub fn part1(input: &Terrain) -> u16 {
    ...
    let end = input.end;
    let tmap = &input.tmap;
    let mut pathmap: Vec<Vec<Option<u16>>> = tmap.iter()
    ...
    #[cfg(test)]
    println!("Min path of {:?} steps found in {} iterations.", steps_to_end, iterations);
...
pub fn part2(input: &Terrain) -> u16 {
    ...
    for y in 0..tmap.len() {
        for x in 0..tmap[0].len() {
            if tmap[y][x] == 0 {pathmap[y][x] = Some(0);}
        }
    }
        ...
        for x in 0..=xmax {
            ...
            for (yy,xx) in adjacent_list {
                ...
                if let Some(adjacent_path) = pathmap[yy][xx] {
                    if this_height > adjacent_height + 1 {continue;}
