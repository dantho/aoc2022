...
pub fn part1(input: &[(bool,Cuboid)]) -> usize {
    ...
    .filter(|(_,c)|c.0.1 <= 50 && c.0.1 >= -50) // c.0.0 DOES NOT WORK!  Look for "=1.." in input file!
    .fold(Vec::new(),|prior_steps: Vec<(bool,Cuboid)>, (new_is_on, new_c):(bool,Cuboid)| {
        let mut updated = prior_steps.into_iter()
...
mod tests {
    ...
    fn test_part1_3() {
        ...
        on x=1..10,y=1..10,z=1..10
        on x=5..5,y=7..9,z=6..15
        on x=4..6,y=8..8,z=6..15
        ");
        let p1 = part1(&g);
        // A cube 10x10x10 with two planes 1x3x10, 3x1x10
        // SUBTRACTING two planes: 1x3x10 and 3x1x10
        // planes overlap each other on a line, overlap the cube on orthogonal planes
        assert_eq!(p1, 1000+15+10);
