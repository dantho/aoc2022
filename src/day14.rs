...
pub fn gen1(input: &str) -> CaveSystem {
    ...
    let terrain_coords = input
        ...
        .collect::<Vec<_>>();
    let mut ymax = terrain_coords
        .iter()
        .map(|v| v.iter())
        .flatten()
        .fold(0, |yy, (_, y)| yy.max(*y));
    ymax += 1; // single blank row at bottom
    let (xmin, xmax) = terrain_coords
        .iter()
        .map(|v| v.iter())
        .flatten()
        .fold((usize::MAX, 0), |(mx, xx), (x, y)| {
            (mx.min(*x), xx.max(*x + *y))
        });
    assert!(xmax >= STARTX);
...
pub fn part1(caves: &CaveSystem) -> usize {
    ...
    caves
        .map
        .iter()
        .map(|v| v.iter().filter(|item| item == &&Sand))
        .flatten()
        .count()
}
...
fn let_sand_fall(mut caves: CaveSystem) -> CaveSystem {
    ...
    while still_filling {
        ...
        while grain != last_loc {
            ...
            caves.map[grain.1][grain.0] = Air;
            grain = match [
                caves.map[down.1][down.0],
                caves.map[leftd.1][leftd.0],
                caves.map[rightd.1][rightd.0],
            ] {
                [Air, _, _] => down,
                [_, Air, _] => leftd,
                [_, _, Air] => rightd,
